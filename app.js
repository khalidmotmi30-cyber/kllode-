...
function now(){const d=new Date();return{date:d.toISOString().slice(0,10),time:d.toLocaleTimeString('en-US',{hour:'2-digit',minute:'2-digit',hour12:true}),day:['الأحد','الاثنين','الثلاثاء','الأربعاء','الخميس','الجمعة','السبت'][d.getDay()]};}
...
function setNow(){const n=now();$('reportDate').value=n.date;$('reportTime').value=n.time.replace(/\s?(AM|PM)$/,'');$('reportDay').value=n.day;updatePreview();}
...
function updatePreview(){const no=$('reportNo').value||'—',day=$('reportDay').value||'—',date=formatEnglishDate($('reportDate').value),time=$('reportTime').value||'—';const byName=new Map(units.map(u=>[u.name,u]));const ordered=FIXED_REPORT_ORDER.map(name=>byName.get(name)).filter(Boolean);$('reportPreview').textContent=`تم تحديث تقرير عمليات ( ساندي و بوليتو ) رقم ( ${no} ) في تمام الساعه ( ${time} ) في يوم ( ${day} ) التاريخ ${date}\n\n`+ordered.map(formatReportSection).join('\n\n');}
